...
my-agent/
  ...
  capabilities/
    calculator.py        # @tool-decorated capability
```

#### Tool Convention

Each `.py` file in `capabilities/` defines tools with the `@tool` decorator
(SPEC-021). The loader discovers any function stamped with
`_arc_capability_meta`:
...
my-agent/
  ...
  capabilities/calculator.py   # @tool-decorated calculator capability
```
...
List all tools discovered in the agent's `capabilities/` directory.

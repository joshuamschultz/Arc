...
1. **Backward-compatible protocol changes**: When updating `Module.startup(bus)` to `Module.startup(ctx: ModuleContext)`, making `ModuleBus.startup(ctx=None)` optional preserved all 575 existing tests without modification. The key pattern: accept both old and new signatures during transition.

2. **Convention loader needs special-case constructors**: The `ModuleLoader.load()` method needed module-specific constructor logic (memory module takes `config`, `eval_config`, `telemetry`, `workspace` while generic modules take no args). A future improvement would be a standardized module constructor interface.

3. **Lazy init avoids circular dependencies**: `_get_eval_model()` with caching solved the eval model initialization problem cleanly. The model is created on first use from either EvalConfig or the agent's LLM config as fallback, avoiding the need to initialize ArcLLM models during module construction.

4. **Frozen dataclass for DI**: Using `@dataclass(frozen=True)` for `ModuleContext` enforces immutability at the type level. Modules get read-only access to bus, tool_registry, config, telemetry, workspace, and llm_config.

5. **Test count progression**: 575 (baseline) -> 580 (Phase 1) -> 592 (Phase 2) -> 599 (Phase 3) -> 602 (Phase 4) -> 606 (Phase 5) -> 610 (Phase 6). Zero regressions across all phases.

6. **Core LOC budget**: Added `module_loader.py` (~120 LOC) bringing core total to 3,418 of 3,500 budget. Only 82 LOC remaining before threshold.

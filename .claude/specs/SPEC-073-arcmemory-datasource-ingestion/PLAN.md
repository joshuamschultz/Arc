...
- [x] **T-1018**: (red) Brain-port ingestion API contract + idempotency + ordering
...
- [x] **T-1019**: (green) Implement Brain-port ingestion API
...
- [x] **T-1020**: (red) IndexBackend conformance + SqliteIndexBackend parity + vec0 scope isolation
...
- [x] **T-1021**: (green) Extract SqliteIndexBackend + vec0 scope filter + factory
...
- [x] **T-1022**: (red) Config: new fields + per-capability off-switches
...
- [x] **T-1023**: (green) Add config fields + degrade wiring
...
- [x] **T-1024**: (red) Source adapter + Router fan-out
...
- [x] **T-1025**: (green) Implement adapter seam + Router
...
- [x] **T-1026**: (red) Memory sink routes through capture
...
- [x] **T-1027**: (green) Implement memory sink
...
- [x] **T-1028**: (red) Extractor + Chunker seams (per-type, sandboxed, sizes)
...
- [x] **T-1029**: (green) Implement Extractor + Chunker
...
- [x] **T-1030**: (red) Per-source hybrid doc index + document_search
...
- [x] **T-1031**: (green) Implement doc indexer + document_search
...
- [x] **T-1032**: (red) Datastore ontology + typed read-only ops + datastore_query
...
- [x] **T-1033**: (green) Implement schema introspection + typed-op generator + datastore_query
...
- [x] **T-1034**: (red) Blob ontology walker (bounded, coarse)
...
- [x] **T-1035**: (green) Implement blob ontology discovery walker
...
- [x] **T-1036**: (red) Classification discovery + document_sanitize + provenance dedup
...
- [x] **T-1037**: (green) Implement classify_remote_object + document_sanitize + provenance table
...
- [x] **T-1038**: (red) Mapping proposal + SPEC-035 approval + mapping-as-facts
...
- [x] **T-1039**: (green) Implement propose_mapping + approval wiring + mapping facts
...
- [x] **T-1040**: (red) Mutability-driven sync + backfill caps
...
- [x] **T-1041**: (green) Implement sync engine + cap enforcement + trigger seam
...
- [x] **T-1042**: (red) Three source-scoped retrieval tools registered
...
- [x] **T-1043**: (green) Register the three tools in the arcagent memory module
...
- [x] **T-1044**: (red) Security envelope on every op
...
- [x] **T-1045**: (green) Wire identity + policy + audit on all paths
...
- [x] **T-1046**: (red) Triad real-path E2E (all three axes)
...
- [x] **T-1047**: (green) Make the triad green end-to-end
...
- [x] **T-1048**: (red) Boundary + determinism guard
...
- [x] **T-1049**: (green) Enforce boundaries + determinism
...
- [x] **T-1050**: (refactor) Degrade + hardening + off-switch verification
  - domain: backend · Components: COMP-014 · Requirements: REQ-383

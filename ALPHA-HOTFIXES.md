...
| H-041 | arcskill | Real skill improvement loop: pick traces → edit ideal → golden set → improve | L | MERGED | 20ef4e8f |
...
| H-047 | Security | build_brain unguarded factory → cross-agent memory bypass (ASI03). Guard: claimed==proven identity + did_matches_pubkey + workspace-owns-DID; fail closed; bootstrap edge; MemoryIsolationError vocab | S | CODING | |
...
Grounded by two research passes (knowledge/connected-data map; arcskill-improver inventory). Cross-cutting: MemoryOperator is the single arcui seam (never `import arcmemory` in arcui — reach via the agent facade like `knowledge_summary`); every new read gates through `gate_no_read_up` + scope key; IndexBackend abstracts the two-DB split; arcui pulls (SPEC-026); new index/semantic state must be durable+idempotent (no expiry — the fixed 24h-mapping class).

- **H-023 (foundation, CODING):** `MemoryOperator.browse_chunks` + `search_chunks(mode=literal|vector)` on IndexBackend primitives; gate + scope every result; cap chunk_text SIZE (not just count); loud degrade. arcui route + Knowledge-tab chunk explorer. *(document_search returns doc HITS, not raw chunks — this is genuine net-new.)*
- **H-016 (CODING):** `MemoryOperator.graph(node,hops)` + new `WeightedGraph.all_edges(scope)`; typed+classified nodes via `_target_type`; an edge to a filtered node DROPS entirely, no leak via counts/degree/IDs (named test); hop+node caps; hover via same gated read; bundled d3-force (no CDN).
- **H-024:** read-only explorer — tables/schema via connector `DatastorePort.introspect` (NOT raw SQL); its chunks via H-023 bounded to the doc-scope; row previews go through H-025's redaction. Targets the **postgres/DATASTORE** connector only.
- **H-025 (DB semantic layer — WIRE the existing one):** call `semantic_layer.overlay()` in `brain.register_datastore` before persist_ontology (thread connection_id — the one plumbing gap; `overlay()` has zero callers today). ADD sample values (`unique.head(10)`) with redact via arctrust.redaction + SECRET_PATTERNS, inherit connection classification, **tier-aware sampling (federal default OFF)**, LIMIT-bounded queries. `datastore_describe` tool so agent hits layer FIRST (compose/prepend, never shadow retrieval). Operator view/edit in arcui — **sign the layer like an arcprompt overlay (operator key + .arcsig, verify-at-load, sign-on-save)**; personal-tier-unsigned-but-audited is a JOSH call to surface, not default.
- **H-026 (OKF doc index — VERIFY + surface):** `DocIndex.sync_collection_index` already writes a local `index.md` at `<workspace>/memory/connected/<source_id>` (never touches smb/dropbox/s3 origin) + registers the routing chunk; `reindex_source` drives it. Confirm it runs on every reindex (no expiry), confirm the ~/arc-hosted location, surface the repo/index view. Targets DOCUMENT/MAIL sources. *(No SPEC-074 exists — nothing pre-specced this.)*
- **H-027 (promote to shared — WIRE + UI):** `FleetSharedKnowledgeComposition.start()` has no production caller — wire it live (promote tool active). Gates already sound (membership + personal-scope + caller==owner + no-write-down + Ed25519/TOFU, mechanical-auto). Verify every promote emits an audit event. arcui shared-knowledge READ view (FleetSharedKnowledgeService.search/read, access-scoped/classification-filtered/revocation-aware) + a filter for what shares + per-agent what-is-currently-shared.
- **H-016b/journey:** cross-cutting connection-card journey test (granted connection → synced → chunk visible in H-023 UI → semantic layer consulted) — the cluster can't close without it (root CLAUDE.md "a data connection is not complete when its tools work").
- **H-042 (gap-closure + docs):** the improver is a real working gated loop that NEVER FIRES (zero shipped skills have evals/, adapter defaults "none"). Build: Curator **consolidate/merge** (only missing mechanism), a reviewable **diff-merge** surface (Hermes gate #5), operator doc, and **seed a real evals/ suite on ≥1 shipped skill + flip the adapter so the loop demonstrably fires** (this proves H-041's substrate). "Never automatic-AND-gated at once" is enforced (machine cases don't count toward the min).

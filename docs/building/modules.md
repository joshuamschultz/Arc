...
flowchart LR
    ...
    A["config.py<br/>typed settings"]:::a
    B["_runtime.py<br/>per-agent bound state"]:::a
    C["capabilities.py<br/>@tool · @hook · @background_task"]:::b
    D["Module Bus<br/>subscribes, never called directly"]:::b
...
own doc — e.g. `docs/tasks-module.md` for `tasks`.
...
This is the file you write for a new module. `echo` below is the module you are
creating — the `_runtime` import resolves inside your own package, exactly as it
does for the shipped modules (`workpad`, `memory`, `tasks`, …).

```python
"""Echo module — the module you are creating, at arcagent/modules/echo/."""
...
- **Per-module facts are declared by the module's own `configure()` signature — core names
 ...
 signing authority unless it explicitly asks for it (/037); the WORM-sink modules

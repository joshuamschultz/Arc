...
```python
from arcgateway_slack import SlackAdapter
...
```python
from arcgateway_mattermost import MattermostAdapter
...
- [DEPLOYMENT.md](../../runbooks/deploy/overview.md) - Production deployment guides
- [SECURITY.md](../../reference/security.md) - Security model
- [IMPLEMENTATION_GUIDES.md](../implementation-guides.md) - Custom adapters
...
> [API reference](../../reference/api.md#arcgateway).

### Classes

| Class | Purpose |
|---|---|
| `AsyncioExecutor` | In-process executor using asyncio tasks. |
| `DeliveryTarget` | Parsed destination for an outbound message. |
| `Delta` | One streamed chunk from an executor run. |
| `Executor` | Contract for running an ArcAgent in response to an InboundEvent. |
| `GatewayRunner` | Supervises platform adapters and routes messages to the SessionRouter. |
| `InboundEvent` | Normalised inbound message from any platform adapter. |
| `SessionRouter` | Routes inbound events to per-session agent tasks. |

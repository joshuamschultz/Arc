...
```python
from arcagent.skills import load_skills
...
memory = MemoryService(agent.store)
...
def load_skills(agent_path: Path, config: AppConfig) -> list[Skill]:
...
- [Implementation Guides](IMPLEMENTATION_GUIDES.md) - Creating capabilities

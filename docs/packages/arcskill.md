...
result = verify_bundle(skill_bundle, issuer_did)
if not result.valid:
...
if not verify_inclusion(result.signature, result.rekor_uuid):
...
```python
from arcskill.crl import check_revocation

if check_revocation(skill_cert_fingerprint):
...
```python
from arcskill.scan import scan
...
- [Security Model](SECURITY.md) - Verification requirements

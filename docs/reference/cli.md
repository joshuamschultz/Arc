...
> **Reference**  ·  Look up  ·  page 2 of 8  
> **For** Anyone looking something up  
> [← API](api.md)  ·  [Docs home](../README.md)  ·  [Configuration keys →](config.md)
...
flowchart LR
    ...
    A["arc"]:::a
    B["agent<br/>build · run · check"]:::b
    C["surfaces<br/>ui · tui · gateway"]:::b
    D["operator<br/>approve · stop · trust"]:::c
    E["content<br/>skill · ext · blueprint · prompt"]:::b
    F["fleet<br/>team · task · memory · store"]:::b
    A --> B & C & D & E & F
```

All commands available via the `arc` console script (installed by `arcmas` or `arccli`).
**See also:** [QUICKSTART.md](../building/quickstart.md), [packages/arccli.md](../building/packages/arccli.md)
...
`@tool`, `@hook`, `@background_task`, or `@capability` — and inspect the four
generalized extension-point families (`brain`, `skills`, `tools`, `hook-builds`;).
...
. `apply` verifies the signature (fail-closed above `personal`), deep-merges the
...
| `arc ui start --team-root <dir>` | Agent-discovery root ( routes); defaults to `./team` if it exists | `arc ui start --team-root ./team` |
...
- [ADR-018](https://github.com/joshuamschultz/Arc/blob/main/.claude/architecture/decisions/ADR-018-no-mcp-no-migration-no-acp.md) — No MCP, No Migration, No ACP

...
| **arcstore** | The filing cabinet for everything an agent did. | Always-on append-only spool plus a PostgreSQL `StorageBackend` query/mutation layer; local PostgreSQL and Supabase are supported; pure reader/writer, no policy logic. | `packages/arcstore/src/arcstore/` |
...
| **StorageBackend** | The interface any database engine plugs into to serve arcstore's queries. | A `Protocol`; `PostgresBackend` is the production implementation, with an in-memory fake for tests. | `packages/arcstore/src/arcstore/backends/base.py:57` |
| **PostgreSQL operational store** | ArcStore's shared query and mutable-record plane. | `PostgresBackend` receives backfilled/tailing spool and WORM records and serves arcui/agent reads and mutations; use a local PostgreSQL database or Supabase. | `packages/arcstore/src/arcstore/backends/postgres.py` |
| **Observe plane** | The read-only side of arcui — watching what agents have done. | PostgreSQL-backed ArcStore queries over the append-only spool and signed WORM sources, served read-on-demand over REST; no live push wire. | `packages/arcui/src/arcui/observe.py:1` |
...
| **proactive recall / `on_moment`** | Memory speaking up unprompted, instead of waiting to be asked. | `Brain.on_moment(kind, ...)` — a model-free detector per moment `kind` decides `fire`, then reuses the normal gated `retrieve()` pass. Returns `""` on a miss, never blocks the turn (SPEC-071). | `packages/arcmemory/src/arcmemory/brain.py:227` |
| **working set** | The entities "in play" in a conversation right now, even if the last message didn't name them. | `WorkingSet` — a bounded, decaying, salience-filtered per-session cue accumulator the `on_moment` detectors read; pure, model-free (SPEC-072). | `packages/arcmemory/src/arcmemory/detectors.py:269` |
| **decision point** | A spot mid-loop, before the agent plans or runs a tool, where memory gets one more chance to speak up. | A `decision_point` moment (`pre_plan` default, `pre_tool` opt-in) staged via `midloop_recall` and drained into the next model call by arcrun's existing `transform_context` hook — no new arcrun mechanism (SPEC-072). | `packages/arcagent/src/arcagent/core/model_manager.py:129` |
| **supersession** | A fact that changed — the old value isn't erased, it's marked as replaced. | `Fact.was_value`: a changed value folds the prior into a retained trail; `superseded_view()` renders "current, DATE \| superseded: old-value" (mark, never delete). | `packages/arcmemory/src/arcmemory/stores/semantic.py:49` |
| **timeline (what changed)** | A dated list of what happened or changed over a period, without a new place to store it. | `read_timeline()` — a chronological, no-read-up-gated view over the *already-existing* event + daily stores (SPEC-072, adds no new store). | `packages/arcmemory/src/arcmemory/timeline.py:35` |

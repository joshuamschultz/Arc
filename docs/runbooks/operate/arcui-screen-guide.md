...
| Connections (Google): `connection.google_account`, `connection.google_client`, `connection.google_access`, `connection.google_sign_in`, `connection.sign_in_address` | The account a connection reads, the operator's own OAuth client name, read-only versus draft-and-send access, the Open Google sign-in step, and the pasted address from the browser bar. | Status reads Working, Reconnect needed, Not signed in or Not installed. Reconnect: Open Google sign-in, sign in as the shown account, paste the address the browser lands on. A blank client warns that sign-ins expire in about 7 days. An agent granted several Google accounts names the account per call and can never reach one it was not granted. After an update changes the tools, click Approve once per Google connection. See [Google accounts](google-accounts.md). |

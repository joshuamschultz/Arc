...
This guide describes the screens currently available in ArcUI. Open a screen from the left navigation. An empty list can mean there is no data yet; a loading or error state means the screen could not retrieve it. Retry after checking the relevant service, then use the screen's visible error details or Audit when available. Field-level contextual help copy is authored in [`screen-help.json`](../../../packages/arcui/web/src/content/screen-help.json); Settings matches exact serialized paths first, then authored `*` patterns that each cover one path segment. Its field-to-control mapping and schema-derived exceptions are listed in the [field-help inventory](../../design/arcui-field-help-inventory.md).

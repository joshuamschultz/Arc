...
> **Runbooks**  ·  Operate  ·  page 10 of 16  
...
flowchart LR
    ...
    A["tool call"]:::a
    B["PolicyPipeline<br/>first-DENY-wins"]:::b
    C["allowed → execute"]:::b
    D["denied → audit + arc approve"]:::c
    E["proactive engine<br/>schedules its own turns"]:::b
    A --> B
    B -->|allow| C
    B -->|deny| D
    E --> A
...
Operational guide for the arc-core-hardening surface: policy pipeline, proactive engine, dynamic tool surface, and tier-aware self-modification.
...
the release; the new engine runs alongside the legacy

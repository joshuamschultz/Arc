...
flowchart LR
    ...
    A["arc team register"]:::a
    B["DID registry"]:::b
    C["mailboxes<br/>pull cursor per agent"]:::b
    D["signed messages<br/>Ed25519"]:::c
    A --> B --> C --> D
```

This is the validated flow for standing up a multi-agent fleet on top of a
single-node deployment ([single-node.md](../deploy/local.md)) — one `arc ui
...
[single-node.md § Dashboard capabilities](../deploy/local.md#dashboard-capabilities-reality-mirror))

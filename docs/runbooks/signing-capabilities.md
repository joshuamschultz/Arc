...
- The **operator key** for this deployment, at `~/.arc/operator`. There is no
...
- Run the approval on a host that holds the operator seed with
  `custody = "in_process"` in that host's `~/.arc/arcagent.toml`, then ship the
...
Custody is read from the **machine** `~/.arc/arcagent.toml`, not from the
...
| The operator key | `~/.arc/operator` |
| The custody setting | `[security] custody` in `~/.arc/arcagent.toml` |

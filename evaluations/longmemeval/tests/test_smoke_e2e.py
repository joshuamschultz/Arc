...
def _live_run_blockers() -> list[str]:
    ...
    if not ORACLE_DATASET.is_file():
        missing.append(
            ...
            "absent; evaluations/longmemeval/data/ is gitignored)"
        )
...
def repo_results_dir(request: pytest.FixtureRequest) -> Iterator[Path]:
    ...
    ``/evaluations/longmemeval/results/`` and ``/evaluations/longmemeval/runs/``, so only artifacts
    ...
    ``runs/`` is created up front because ``/evaluations/longmemeval/runs/`` carries a

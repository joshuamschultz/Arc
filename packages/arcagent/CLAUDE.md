...
- Memory, sessions, `context.md`, identity, and the audit chain are written with **direct filesystem I/O to the workspace** (`Path.write_text` / `atomic_write_text`), NOT by calling the builtin `write`/`bash`/`edit` tools.
- **Why:** the builtin file/exec tools are rooted at `working_dir` — the workspace by default, but the *trusted launch project* for an opt-in coding agent (`[tools] operate_in_launch_dir`). Persisting agent state via those tools would write the agent's brain into whatever project you're coding in. Direct workspace I/O is what lets the tools move to your project while memory stays home. (ADR-029)
- Rule for any new module/capability: writing **project** files via the tools = correct; saving **agent** state = use the workspace path directly. `working_dir` moves the tool root, never the fence (boundary stays `workspace + allowed_paths`).

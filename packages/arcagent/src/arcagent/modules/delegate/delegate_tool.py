...
def make_delegate_tool(
    *,
    ...
    async def _execute(params: dict[str, Any], ctx: ToolContext) -> str:
        ...
        # ctx.parent_state MUST be set by executor.py before invoking the delegate
        # tool.  Callers that pass None bypass depth enforcement — we fail fast
        # here rather than silently defaulting to depth=0 (TD-036).
        if ctx.parent_state is None:
            raise ValueError(
                "delegate_tool invoked without ctx.parent_state; "
                "caller must populate in arcrun.loop or orchestrator"
            )
        parent_depth: int = ctx.parent_state.depth
        ...
        # parent_state is guaranteed non-None here (ValueError raised above).
        # Use the live RunState so spawn() inherits correct depth, max_depth,
        # token budget, and event bus from the parent turn.

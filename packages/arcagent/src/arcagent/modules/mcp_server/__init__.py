...
The full door is built here: ``server`` (``server/discover`` + ``tools/list``),
``identity`` (signed-envelope verify), ``enrollment`` (fleet-member roster),
``allowlist`` (per-tier tool exposure), ``dispatch``/``door`` (the
verify → enroll → allowlist → dispatch → audit pipeline), ``audit``, and the
``router``/``http_transport``/``stdio_transport`` wire transports. The module's
own ``capabilities.py`` builds the read-only listing surface in-process;
``serving.build_door_from_agent`` assembles the full serving door that
``arc mcp serve`` binds to a socket (arcagent stays headless).
"""

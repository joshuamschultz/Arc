...
class RuncontrolConfig(ModuleConfig):
    ...
    data_dir: str = ""
    # A pending request that never matches a live run (a run that already ended
    # before the watcher saw it, or one that never started) is swept to ``expired``
    # once it is older than this. Default gives a run ample time to start before
    # its cancel request is aged out.

...
# Cloud providers that are never allowed at federal tier
_CLOUD_STT = frozenset({"whisper_api", "openai_whisper"})
_CLOUD_TTS = frozenset({"elevenlabs"})
...
class VoiceModule:
    ...
    def _build_stt_provider(self) -> STTProvider:
        ...
        if name in ("whisper_cpp",):
            ...
            # Pass model_path only when the config value looks like a
            # filesystem path (contains a '/'). A bare name like 'base.en'
            # is not a path — _locate_model falls back to the default cache.
            _wcpp_model_cfg = self._config.whisper_cpp_model
            _wcpp_model_path = _wcpp_model_cfg if '/' in _wcpp_model_cfg else None
    ...
    def _build_tts_provider(self) -> TTSProvider:
        ...
        if name in ("piper",):
            ...
            # Pass voice_path only when the config value looks like a
            # filesystem path. A bare name like 'en_US-lessac-medium' is
            # not a path — _locate_voice falls back to the default cache.
            _piper_voice_cfg = self._config.piper_model
            _piper_voice_path = _piper_voice_cfg if '/' in _piper_voice_cfg else None

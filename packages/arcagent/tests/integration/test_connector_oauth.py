...
def _connections(tmp_path: Path, root: Path) -> Connections:
...
async def _install_with_app_creds(tmp_path: Path, root: Path) -> None:
...
async def test_complete_oauth_stores_a_durable_refresh_token_and_connects(
    ...
    await _install_with_app_creds(tmp_path, root)
    monkeypatch.setattr("arcagent.connections._oauth_post", _ok_post)

    auth = await _connections(tmp_path, root).complete_oauth(_INSTANCE, code="one-time-code")
...
async def test_authorization_offers_the_url_and_never_asks_for_the_managed_token(
    ...
    auth = await _connections(tmp_path, root).authorization(_INSTANCE)
...
async def test_a_dead_code_refuses_and_stores_nothing(
    ...
    with pytest.raises(ExtensionError) as exc:
        await _connections(tmp_path, root).complete_oauth(_INSTANCE, code="expired")

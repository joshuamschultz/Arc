...
4. ctx.parent_state=None (legacy path) → falls back to depth=0 gracefully
...
class TestDepthCapRejection:
    ...
    @pytest.mark.asyncio
    async def test_parent_state_none_falls_back_to_depth_0(self) -> None:
        """Legacy path: ctx.parent_state=None → depth treated as 0, no crash."""
        cfg = DelegateConfig(max_depth=2)
        ...
        with patch(
            ...
            raw = await tool.execute({"task": "legacy task"}, ctx)
            result = json.loads(raw)

        # Should not be rejected by depth check
        assert result.get("error") != "max_depth"

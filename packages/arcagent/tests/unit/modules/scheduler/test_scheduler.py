...
class TestShouldFireInterval:
    def test_fires_when_elapsed(self) -> None:
        engine = SchedulerEngine(
            store=MagicMock(), config=make_config(),
            telemetry=MagicMock(), agent_run_fn=AsyncMock(),
        )
        entry = make_entry(
            every_seconds=300,
    ...
    def test_timezone_conversion(self) -> None:
        ...
        entry = make_entry(
            active_hours=ActiveHours(
                start="08:00", end="18:00", timezone="US/Eastern",
            ),
        )
        assert engine.is_within_active_hours(entry) is True
...
class TestCircuitBreaker:
    def test_disables_after_threshold(self) -> None:
        ...
        engine = SchedulerEngine(
            store=store, config=make_config(),
            telemetry=MagicMock(), agent_run_fn=AsyncMock(),
        )
...
class TestOnceAutoDisable:
    ...
    async def test_once_disabled_after_execution(self) -> None:
        ...
        engine = SchedulerEngine(
            store=store, config=make_config(),
            telemetry=MagicMock(), agent_run_fn=agent_run_fn,
        )
        entry = ScheduleEntry(
            id="sched_once",
            type="once",
...
class TestExecution:
    ...
    async def test_execute_handles_timeout(self) -> None:
        ...
        engine = SchedulerEngine(
            ...
            telemetry=MagicMock(), agent_run_fn=slow_run,
        )
...
class TestLifecycle:
    ...
    async def test_stop_drains_queue(self) -> None:
        ...
        engine = SchedulerEngine(
            store=store, config=make_config(),
            telemetry=MagicMock(), agent_run_fn=mock_run,
        )
        await engine.start()
    ...
    async def test_set_agent_run_fn(self) -> None:
        ...
        engine = SchedulerEngine(
            store=MagicMock(), config=make_config(),
            telemetry=MagicMock(), agent_run_fn=original_fn,
        )

"""SPEC-056 Phase B — ``arcagent.modules.tasks`` tool surface — RED.
...
@pytest.fixture
async def tasks_state(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> AsyncIterator[Any]:
    """Bootstrap the live runtime against a tmp_path SQLite db and yield the state.
...
@pytest.mark.asyncio
class TestListTasks:
    async def test_scope_self_filters_to_own_did(
        self, peer_state: Any, tasks_state: Any
    ) -> None:
...
class TestDecomposeTask:
    async def test_decompose_creates_subtasks_and_blocks_parent(
        self, tasks_state: Any
    ) -> None:
...
class TestOwnerOnlyEnforcement:
    ...
    async def test_fail_task_rejected_for_non_owner(
        ...
        owned_by_bob = json.loads(await create_task(title="Bob's work", owner="@bob"))
        result = json.loads(
            await fail_task(id=owned_by_bob["id"], resolution="sabotage")
        )
        assert "error" in result

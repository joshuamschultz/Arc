...
class TestGrepTool:
    ...
    async def test_grep_simple_pattern(self, workspace: Path, grep_tool: Any) -> None:
        (workspace / "code.py").write_text("def hello():\n    return 'world'\n")
        result = await grep_tool(pattern="hello")
        assert "code.py:1:" in result
        assert "def hello" in result

    async def test_grep_regex_pattern(self, workspace: Path, grep_tool: Any) -> None:
    ...
    async def test_grep_no_matches_returns_message(self, workspace: Path, grep_tool: Any) -> None:
        (workspace / "empty_match.txt").write_text("nothing here\n")
        result = await grep_tool(pattern="zzzznotfound")
        assert "No matches" in result

    async def test_grep_in_subdirectory(self, workspace: Path, grep_tool: Any) -> None:
        sub = workspace / "sub"
        sub.mkdir()
    ...
    async def test_grep_with_path_filter(self, workspace: Path, grep_tool: Any) -> None:
    ...
    async def test_grep_with_glob_filter(self, workspace: Path, grep_tool: Any) -> None:
    ...
    async def test_grep_max_results_limit(self, workspace: Path, grep_tool: Any) -> None:
    ...
    async def test_grep_skips_binary_files(self, workspace: Path, grep_tool: Any) -> None:
    ...
    async def test_grep_empty_workspace(self, workspace: Path, grep_tool: Any) -> None:
...
class TestGrepReDoSProtection:
    ...
    async def test_pattern_at_max_length_accepted(self, workspace: Path, grep_tool: Any) -> None:
        (workspace / "file.txt").write_text("a" * 1000 + "\n")
        pattern = "a" * 1000
        result = await grep_tool(pattern=pattern)
        assert "Error" not in result

    async def test_invalid_regex_pattern(self, workspace: Path, grep_tool: Any) -> None:
...
class TestGrepErrorHandling:
    ...
    async def test_stat_failure_skips_file(self, workspace: Path, grep_tool: Any) -> None:
    ...
    async def test_large_file_skipped(self, workspace: Path, grep_tool: Any) -> None:
    ...
    async def test_read_failure_skips_file(self, workspace: Path, grep_tool: Any) -> None:
    ...
    async def test_unicode_decode_error_skips_file(self, workspace: Path, grep_tool: Any) -> None:

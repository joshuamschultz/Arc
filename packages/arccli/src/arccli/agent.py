...
@click.option("--ui", is_flag=True, help="Enable UI reporter module (streams events to ArcUI).")
def agent_serve(path: str, verbose: bool, ui: bool) -> None:
    ...
    Use --ui to enable the UI reporter module, which streams agent events
    to a standalone ArcUI server (started separately via ``arc ui start``).
    ...
    try:
        loop.run_until_complete(
            _serve(agent_dir, shutdown_event, verbose, ui=ui)
        )
    finally:
        loop.close()
...
async def _serve(
    ...
    *,
    ...
    # Enable UI reporter module if --ui flag is set
    if ui:
...
def _enable_ui_reporter(config: Any) -> None:
    ...
    Creates a proper ModuleEntry with enabled=True. Token resolution
    is handled by the UIReporterModule itself (config > env > file).
    """
...
async def _chat_interactive_arcagent(
    *,
    ...
    # SPEC-017 R-003: REPL-mutable state (was display-only).
    # ``active_sandbox`` is a list of tool names the agent may call on
    # subsequent turns; empty means no restriction. ``active_strategy``
    # is the strategy name for the next chat() call. Both seed from
    # CLI flags and can be changed mid-session.
    active_sandbox: list[str] | None = (
        [t.strip() for t in sandbox_tools.split(",") if t.strip()]
        if sandbox_tools
        else None
    )
    active_strategy: str | None = strategy
    ...
    try:
        while True:
            ...
            if user_input.startswith("/sandbox"):
                ...
                if arg:
                    active_sandbox = [t.strip() for t in arg.split(",") if t.strip()]
                    click_echo(f"  Sandbox set: {','.join(active_sandbox)}")
                else:
                    active_sandbox = None
                    click_echo("  Sandbox cleared (all tools allowed)")
                # R-003: audit the change so operators can trace REPL actions
                if arc_agent._telemetry is not None:
                    arc_agent._telemetry.audit_event(
                        "repl.sandbox_changed",
                        {"sandbox": active_sandbox, "caller": "cli_repl"},
                    )
                continue
            ...
            if user_input.startswith("/strategy"):
                ...
                if arc_agent._telemetry is not None:
                    arc_agent._telemetry.audit_event(
                        "repl.strategy_changed",
                        {"strategy": active_strategy, "caller": "cli_repl"},
                    )
                continue
...
# ---------------------------------------------------------------------------
# SPEC-017 Phase 8 — CLI mirror for policy / tool / skill / schedule /
# completion. All commands are thin wrappers around the core APIs. They
# favor scriptability (JSON output) over decoration so CI pipelines can
# consume them.
# ---------------------------------------------------------------------------
...
@click.option("--tier", type=click.Choice(["federal", "enterprise", "personal"]), default="personal")
@click.option("--tool", "tool_name", required=True, help="Tool name to evaluate.")
@click.option("--agent-did", "agent_did", default="did:arc:cli")
@click.option(
    "--classification", default="unclassified", show_default=True,
)
def policy_evaluate(
...
def schedule_migrate(path: str, dry_run: bool) -> None:
    ...
    ``workspace/proactive/schedules.json`` in the new format. Exits
    non-zero if the legacy directory is missing — confirms users
    aren't running it on a clean install.
    """
    from pathlib import Path as _Path
...
agent.add_command(policy_group)
agent.add_command(completion_group)
agent.add_command(schedule_group)

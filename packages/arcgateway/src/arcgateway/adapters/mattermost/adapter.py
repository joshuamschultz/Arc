...
Design (mirroring SlackAdapter, per ADR-002):
  ...
  D-005  Session identity is SessionRouter's, never this adapter's (REQ-304).
...
#: Bound on any single outbound HTTP call (download or post).
...
class MattermostAdapter:
    ...
    supports: tuple[str, ...] = ("text",)
    """Text only — every artefact degrades to a line naming it (REQ-311).

    One source with the descriptor, so the contract suite and ``send`` cannot
    disagree about what this platform will carry.
    """

    text_boundaries: tuple[str, ...] = ("\n\n", "\n", " ")
    """Most posts are markdown; the space boundary avoids cutting mid-word."""
    ...
    async def send(
        ...
        *,
        ...
        """Enqueue a reply for async delivery.  Drop-oldest on full queue.

        This adapter posts text only (``PLATFORM.supports`` says so), so an
        artefact arrives on the channel as a line naming the file and where it
        is — the operator can still fetch it, and the turn is not lost (REQ-311).
        """
        ...
        for part in _as_parts(parts):
            text = (
                ...
                else describe_undeliverable(part, "Mattermost")
            )
    ...
    async def _ensure_http_session(self) -> Any:
        ...
        if self._http_session is None or self._http_session.closed:
            ...
            # Explicit bound: aiohttp defaults to five minutes, long enough for
            # one stalled remote to hold a session task well past any use.
    ...
    def to_parts(self, payload: Any) -> list[DraftPart]:
        """Turn one Mattermost post into ordered parts (REQ-296).

        An attachment-only post carries an empty ``message``, which is why
        reading ``post.message`` alone drops exactly the posts a user expects
        the agent to look at.

        Args:
            payload: The decoded Mattermost post dict.

        Returns:
            The post text (if any) followed by one part per attached file.
        """
        ...
        for attachment in metadata.get("files", []) or []:
            if isinstance(attachment, dict):
                parts.append(self._pending(attachment))
        return parts

    def _pending(self, attachment: dict[str, Any]) -> PendingMedia:
        """Describe one attachment, with a closure that fetches it on demand."""
        mime = str(attachment.get("mime_type") or "application/octet-stream")
        declared_name = str(attachment.get("name") or "file")
        file_id = str(attachment.get("id") or "")
        size = attachment.get("size")

        async def fetch(limit_bytes: int) -> bytes:
            return await self._download(file_id, limit_bytes=limit_bytes)
    ...
    async def _download(self, file_id: str, *, limit_bytes: int) -> bytes:
        """Read an attachment's bytes off the Mattermost REST API, bounded.

        Raises:
            MediaTooLargeOnWireError: The body passed ``limit_bytes``.
        """
        session = await self._ensure_http_session()
        url = f"{self._server_url}/api/v4/files/{file_id}"

...
from arcgateway.adapters._text import split_message
...
async def _ignore_event(event: InboundEvent) -> None:
    """Typed no-op handler for constructor-guard tests that never dispatch."""
...
class TestFederalAirgapGuard:
    ...
    def test_public_host_rejected_at_federal_tier(self) -> None:
        ...
        """
        with patch(
            "arcgateway.adapters.mattermost.adapter.socket.getaddrinfo",
...
# ---------------------------------------------------------------------------
# Session key contract (REQ-304 / REQ-310)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
class TestSessionKey:
    async def test_adapter_supplies_no_session_key(self) -> None:
        """The adapter carries platform identity only; the router owns the key.

        A ``mattermost:{channel}:{user}`` key composed here would be a second
        identity for the same (agent, user) pair and would skip the rotation
        generation, so ``/new`` would not take effect on Mattermost.
        """
        ...
        assert not hasattr(received[0], "session_key")
        assert received[0].chat_id == "ch-abc"
...
class TestInboundDispatching:
    async def test_posted_event_dispatched(self) -> None:
        ...
        assert len(received) == 1
        assert flatten_text(received[0].parts) == "hello agent"

...
class FakeWebAdapter:
    ...
    name: str = "web"
    agent_did: str = ""  # web fronts every agent — see BasePlatformAdapter
...
"""
    ...
    # adapters/slack), landing in the generic ``adapters`` tuple.
...
async def test_slack_and_web_events_share_session_router(
    ...
    land in the single shared router, which is the sole owner of session
    identity (REQ-304): the Slack adapter carries platform identity only and
    ...
    # Slack event: no key at all. A "slack:{channel}:{user_id}" key composed in
    # the adapter would be a second identity for the same (agent, user) pair and
    # would skip the rotation generation, so /new would not take effect there.
    assert not hasattr(s_event, "session_key"), (
        "Slack adapter hand-crafted a session key; session identity belongs to "
        "SessionRouter (REQ-304/REQ-310)"
    )
    assert s_event.chat_id == slack_channel

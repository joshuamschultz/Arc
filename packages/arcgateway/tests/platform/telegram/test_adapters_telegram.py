"""Unit tests for arcgateway.adapters.telegram.adapter.TelegramAdapter.
...
Test coverage:
    ...
    - split_for_platform(TelegramAdapter, ) boundary logic
...
from arcgateway.adapters.telegram.adapter import (
    TelegramAdapter,
    _is_conflict_error,
    _is_network_error,
    _network_backoff,
...
def _make_update(user_id: int = 42, text: str = "hello", update_id: int = 1) -> MagicMock:
    ...
    # Explicitly empty, as python-telegram-bot delivers them. A bare MagicMock
    # answers every attribute with a truthy mock, so a text-only fixture would
    # otherwise look like it carried a photo, a document and a voice note.
...
# ── message splitting (the gateway's, per the platform's declaration) ────────


def test_split_message_short_text_returns_single_chunk() -> None:
    chunks = split_for_platform(TelegramAdapter, "hello world")
    assert chunks == ["hello world"]


def test_split_message_empty_text_returns_empty_list() -> None:
    assert split_for_platform(TelegramAdapter, "") == []


def test_split_message_splits_at_paragraph_boundary() -> None:
    text = ("A" * 3000) + "\n\n" + ("B" * 2000)
    chunks = split_for_platform(TelegramAdapter, text)
    assert len(chunks) == 2
...
async def test_message_received_calls_on_message() -> None:
    """A real message becomes an InboundDraft and reaches on_message."""
    on_message = AsyncMock()
    ...
    event: InboundDraft = call_args[0]
    assert isinstance(event, InboundDraft)
    assert event.platform == "telegram"
    assert flatten_text(event.parts) == "hello agent"
...
@pytest.mark.asyncio
async def test_update_carrying_nothing_at_all_is_skipped() -> None:
    """An update with no words and no artefacts has nothing to run on."""
    on_message = AsyncMock()
    adapter = _make_adapter(allowed_user_ids=[42], on_message=on_message)
    adapter._bot_id = 999
...
@pytest.mark.asyncio
async def test_update_with_a_photo_and_no_text_is_forwarded() -> None:
    """SPEC-065: a photo is a message. Skipping it was the reported defect.

    This is the counterpart to the test above, and the pair is the point: an
    update is skipped for being *empty*, never for being "not text".
    """
    on_message = AsyncMock()
    adapter = _make_adapter(allowed_user_ids=[42], on_message=on_message)
    adapter._bot_id = 999
    ...
    on_message.assert_called_once()
    parts = on_message.call_args[0][0].parts

...
async def test_polling_conflict_bounded_retries() -> None:
    ...
    with (
        patch(
            "arcgateway.adapters.telegram.adapter._is_conflict_error",
            side_effect=lambda e: isinstance(e, _FakeConflictError),
        ),
        patch(
            "arcgateway.adapters.telegram.adapter._is_network_error",
            side_effect=lambda e: isinstance(e, _FakeNetworkError),
        ),
        patch(
            "arcgateway.adapters.telegram.adapter.asyncio.sleep",
...
async def test_wait_closed_returns_fatal_state_after_polling_death() -> None:
    ...
    with (
        ...
        patch("arcgateway.adapters.telegram.adapter.asyncio.sleep", new_callable=AsyncMock),
        patch.dict(
            "sys.modules",
            {
                "telegram": MagicMock(),
...
async def test_on_error_escalates_persistent_getupdates_failure() -> None:
    ...
    from arcgateway.adapters.telegram.adapter import _MAX_CONSECUTIVE_UPDATE_ERRORS
    ...
    with (
        ...
        patch("arcgateway.adapters.telegram.adapter._is_conflict_error", return_value=False),
    ):
...
async def test_auth_rejection_emits_audit_log(caplog: Any) -> None:
    ...
    with caplog.at_level(logging.INFO, logger="arcgateway.adapters.telegram.adapter"):

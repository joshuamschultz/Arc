...
async def test_federal_missing_approver_did_increments_lockout_counter(
    ...
    """5 no-DID approvals against any code → platform lockout.

    We pass a bogus code with ``platform_hint="slack"`` so the failure is
    attributed to ``slack`` without needing a pending pairing row (the
    new federal-missing-DID path fails BEFORE touching the row).  This
    also proves probing with invented codes cannot bypass the lockout.
    """
    for _ in range(5):
        with pytest.raises(PairingSignatureInvalid):
            await federal_store.verify_and_consume(
                "NOPECODE",  # never minted; the sig-missing check fires first
                approver_did=None,
                signature=None,
                platform_hint="slack",
            )

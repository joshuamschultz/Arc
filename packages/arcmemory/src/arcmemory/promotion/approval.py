...
class PromotionApprovalQueue:
    ...
    def _id(self, item_id: str) -> str:
        return "promotion:" + hashlib.sha256(f"{self._agent_did}\0{item_id}".encode()).hexdigest()
    ...
    def _verify_grant(
        ...
        *,
        ...
        return (
            ...
            and arctrust.verify_approval_for_hash(expected, grant, agent_did=self._agent_did)
        )
    ...
    async def approve(self, item_id: str, *, grant: arctrust.ApprovalGrant) -> None:
        ...
        if (
            await self._store.resolve(
                self._id(item_id),
                status="approved",
                actor_did=grant.approver_did,
                resolved_by=grant.approver_did,
                grant=arctrust.grant_to_wire(grant),
            )
            is None
        ):

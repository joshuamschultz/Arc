...
class TestDepthLimit:
    @pytest.mark.asyncio
    async def test_spawn_not_available_at_max_depth(self):
        """spawn_task should not appear in tools at max_depth."""
        model = MockModel(
            [
                LLMResponse(content="At max depth.", stop_reason="end_turn"),
            ]
        )
        ...
        assert result.content == "At max depth."

        # The model should have been called with tools that do NOT include spawn_task
...
class TestToolSubsetting:
    ...
    async def test_child_gets_only_named_tools(self):
        ...
        # Child tools should include echo and spawn_task (depth 1 < max 3)
        # but NOT upper
        child_tools = tool_names_seen[1]
        assert "echo" in child_tools
        assert "spawn_task" in child_tools

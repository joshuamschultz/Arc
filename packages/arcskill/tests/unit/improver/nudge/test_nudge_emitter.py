...
from arcskill.improver.nudge.dedup import compute_tool_sequence_hash
from arcskill.improver.nudge.nudge_emitter import (
...
class _FakeCtx:
    """Duck-typed EventContext stand-in (arcskill.improver is bus-agnostic)."""

    def __init__(self, *, event: str, data: dict[str, Any], agent_did: str, trace_id: str) -> None:
        self.event = event
        self.data = data
...
def _make_config(**kwargs: Any) -> ImproverConfig:
    return ImproverConfig(**kwargs)
...
class TestDedup:
    ...
    async def test_dedup_name_collision_blocks_nudge(self) -> None:
        ...
        # Inject known skill names that will collide with the derived name:
        # _derive_proposed_name of the tool set -> "skill-bash-grep-read-run-search"

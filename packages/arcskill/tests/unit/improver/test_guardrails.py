...
from arcskill.improver.models import Candidate, SkillTrace, ToolCallRecord
...
class TestCheckEligible:
    ...
    def test_eligible_with_enough_traces(self) -> None:
        config = ImproverConfig(min_traces=5)
        g = Guardrails(config)
    ...
    def test_eligible_non_exempt_tag(self) -> None:
        config = ImproverConfig(min_traces=5, exempt_tags=["auth"])
        g = Guardrails(config)
        traces = [_make_trace(turn=i) for i in range(10)]
        assert (
            g.check_eligible(
    ...
    def test_not_eligible_generation_limit(self) -> None:
        config = ImproverConfig(min_traces=5, max_generations=10)
        g = Guardrails(config)
...
class TestValidateCandidate:
    ...
    def test_valid_candidate_passes(self) -> None:
        config = ImproverConfig(max_token_ratio=1.5, anchor_distance_threshold=0.15)
        g = Guardrails(config)
    ...
    def test_rejected_token_budget_exceeded(self) -> None:
        config = ImproverConfig(max_token_ratio=1.5)
        g = Guardrails(config)
    ...
    def test_rejected_anchor_distance_exceeded(self) -> None:
        config = ImproverConfig(anchor_distance_threshold=0.15)
        g = Guardrails(config)
        seed = _make_candidate(SEED_TEXT, candidate_id="seed", token_count=SEED_TOKEN_COUNT)

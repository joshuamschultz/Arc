...
class TestRegisterAndEntities:
    ...
    def test_register_agent(
        ...
        main([*_r(tmp_path), "register", "agent://a1", "--name", "A1", "--type", "agent"])
...
class TestSendAndInbox:
    ...
    def _setup_agents(self, root: Path) -> None:
        ...
        main([*_r(root), "channel", "ops", "--members", "agent://a1,agent://a2"])
    ...
    def test_inbox_json(
        ...
        self._setup_agents(tmp_path)
        main(
            [*_r(tmp_path), "--as", "agent://a1", "send", "--to", "channel://ops", "--body", "Hi"]
        )
        capsys.readouterr()
    ...
    def test_send_dm(
        ...
        main([*_r(tmp_path), "register", "agent://a1", "--name", "A1", "--type", "agent"])
        main([*_r(tmp_path), "register", "user://josh", "--name", "Josh", "--type", "user"])
        capsys.readouterr()
...
class TestReadAndThread:
    ...
    def test_read_channel(
        ...
        main([*_r(tmp_path), "--as", "agent://a1", "read", "--channel", "ops"])
        out = capsys.readouterr().out
    ...
    def test_read_channel_json(
        ...
        main([*_r(tmp_path), "--as", "agent://a1", "--json", "read", "--channel", "ops"])
        out = capsys.readouterr().out
        data = json.loads(out)
        assert isinstance(data, list)
    ...
    def test_read_dm(
        ...
        main([*_r(tmp_path), "--as", "agent://a1", "read", "--dm", "a1"])
        out = capsys.readouterr().out
    ...
    def test_thread_view(
        ...
        main(
            [*_r(tmp_path), "--as", "agent://a1", "thread", msg_id, "--stream", "arc.channel.ops"]
        )
        out = capsys.readouterr().out

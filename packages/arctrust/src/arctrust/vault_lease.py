...
class _LeaseClient(httpx.Client):
    ...
    def establish(self) -> None:
        try:
            response = super().request(
                "GET", "/v1/auth/token/lookup-self", timeout=5, follow_redirects=False
            )
    ...
    def _renew(self) -> None:
        try:
            ...
            if auth["renewable"] is not True or type(ttl) is not int or ttl < 1 or ttl > 3600:
    ...
    def revoke_and_close(self) -> None:
        ...
        try:
            super().request(
                "POST", "/v1/auth/token/revoke-self", timeout=5, follow_redirects=False
            )
    ...
    *,
    ...
    try:
        ...
        if not verify_signature(
            ED25519, _DOMAIN + grant.canonical_bytes(), signature, trusted_grant_key
        ):
            raise ValueError("grant signature refused")
        if (
            grant.deployment_id != config.deployment_id
            or grant.tenant_id != config.tenant_id
            or grant.config_revision != config.revision

...
_SAFE_KEY = {
    "type": "aes256-gcm96", "exportable": False, "derived": False,
...
def test_seal_and_open_send_bound_aad_and_no_key_material() -> None:
    ...
    def respond(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(200, json={"data": {"plaintext": base64.b64encode(b"hello").decode()}})

    cipher = VaultCipher(_client(httpx.MockTransport(respond)), mount="transit", key="users-key",
                         scope="tenant/order", record_id="users")
...
def test_unsafe_key_metadata_is_refused(unsafe: dict[str, object]) -> None:
    ...
    with pytest.raises(VaultCipherError, match="unsafe"):
        VaultCipher(client, mount="transit", key="users-key", scope="tenant/order", record_id="users")


def test_redirect_and_transport_error_are_sanitized() -> None:
    client = _client(httpx.MockTransport(lambda _: httpx.Response(302, headers={"location": "https://evil"})))
...
def test_ciphertext_shape_and_bound_are_checked() -> None:
    ...
    cipher = VaultCipher(client, mount="transit", key="users-key", scope="tenant/order", record_id="users")

...
async def get_catalog(request: Request) -> JSONResponse:
    ...
    return JSONResponse(
        ConnectorCatalogResponse(
            available=[_catalog_entry(entry) for entry in entries if not entry.error],
            unreadable=[
...
def _catalog_entry(entry: CatalogEntry) -> ConnectorCatalogEntry:
    ...
    return ConnectorCatalogEntry(
        ...
        host_requires=[
            ConnectorHostRequirement(name=required.name, instruction=required.instruction)
            for required in entry.host_requires
        ],
        tools=[
            ConnectorTool(
                name=tool.name,

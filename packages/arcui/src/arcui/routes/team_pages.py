...
async def get_tasks(request: Request) -> JSONResponse:
    """GET /api/team/tasks — bounded immutable-order pages with continuation."""
    ...
    time_scope = request.query_params.get("time_scope", "all")
    filters = (status, priority, owner_did, tag, time_scope)
    ...
    try:
        ...
        if any(value is not None and len(value) > 200 for value in (owner_did, tag)):
            raise ValueError
        if time_scope not in {"1", "7", "30", "all"}:
            raise ValueError
        before, since = (
            ...
            if cursor is not None
                ...
                None,
                (datetime.now(UTC) - timedelta(days=int(time_scope))).isoformat()
                if time_scope != "all"
                else None,
            )
        )
    ...
    except TaskBoardUnavailableError:
        return JSONResponse(
            ErrorResponse(error="Task board backend unavailable.").model_dump(), status_code=503
        )
    rows = page[:limit]
    next_cursor: str | None = None
    if len(page) > limit:
        last = rows[-1]
        next_cursor = _encode_task_cursor(last.created_at or "", last.id, filters, since)
    ...
    return JSONResponse(
        TaskBoardResponse.model_validate(
            {
                "tasks": out,
...
def _decode_task_cursor(
    ...
    if not isinstance(payload, list) or len(payload) != 8 or tuple(payload[2:7]) != filters:
        raise ValueError("invalid cursor")
    stamp, task_id = payload[:2]
    ...
    since = payload[7]
    if (filters[4] == "all") != (since is None):
        raise ValueError("invalid cursor scope")
    if since is not None:
        if not isinstance(since, str) or datetime.fromisoformat(since).tzinfo is None:
            raise ValueError("invalid cursor scope")
    return (stamp, task_id), since

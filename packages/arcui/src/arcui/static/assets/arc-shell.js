...
const ICONS = {
  ...
  artifacts: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>',
...
const PAGES = [
  ...
  { divider: true },
  // External: agent-generated evidence packages (PDFs + POA&M CSVs)
  // served by Caddy at /artifacts/. Opens in a new tab so the
  // dashboard doesn't lose state.
  { id: 'artifacts', label: 'Artifacts', icon: 'artifacts',
    href: '/artifacts/', external: true },
...
function renderShell() {
  ...
  if (sidebar) {
    const items = PAGES.map(p => {
      ...
      if (p.hidden) return '';
      // External items render as a real anchor that opens in a new tab
      // (e.g. /artifacts/ served by Caddy outside the SPA). Internal
      // items navigate via initSidebarNav's click handler.
      if (p.external && p.href) {
        return `<a class="sidebar-item" href="${p.href}" target="_blank" rel="noopener" title="${p.label}">${ICONS[p.icon] || ''}<span class="sidebar-tooltip">${p.label}</span></a>`;
      }

...
async def test_hub_deduplicates_and_bounds_events() -> None:
    ...
    make = lambda event_id: ApprovalNotification(
        event_id=event_id, approval_id=event_id, status="pending",
        agent_did=None, tool="tool", classification="UNCLASSIFIED", attempts=1,
    )
    await hub(make("one"))
    await hub(make("one"))
    await hub(make("two"))

...
@pytest.mark.parametrize("body", ["null", "[]", '"name"', '{"pairings": []}'])
def test_profile_rejects_non_object_json(client, body):
    token = client.post(
        "/api/auth/login", json={"email": "boss@example.com", "password": GOOD}
    ).json()["token"]
    resp = client.patch(
...
def test_profile_validation_failure_does_not_partially_save(client):
    watcher = client.post(
        "/api/auth/login", json={"email": "watcher@example.com", "password": GOOD}
    ).json()["token"]
    client.patch(
    ...
    assert client.get("/api/auth/me", headers=headers).json()["display_name"] == ""

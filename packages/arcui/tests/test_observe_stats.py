...
class TestLLMByIdentity:
    def test_llm_by_identity(self) -> None:
        ...
        rows = [
            ...
            _row(
                agent_label="researcher:d1", cost_usd=0.01, prompt_tokens=40, completion_tokens=10
            ),
            _row(
                agent_label="researcher:d1", cost_usd=0.03, prompt_tokens=60, completion_tokens=20
            ),
        ]
...
class TestComputeRuns:
    ...
    def test_budget_capped_run_is_limited_not_completed_or_error(self) -> None:
        ...
        # It reached a terminal by hitting a ceiling, so it must NOT read a clean
        # "completed" — but it also did real work and did not die, so it is not the
        # red "error" reserved for a run that errored and never completed. Its own
        # honest status is "limited": raise the cap, or read it as a runaway.
        events = self._events()
        events.append(
            {
                "kind": "run_event",
        ...
        events = [
            {
                ...
                "actor_did": "did:a",
                "name": "turn.start",
                "ts": f"2026-05-31T00:00:0{i}+00:00",
            }
            for i in range(5)
        ]
